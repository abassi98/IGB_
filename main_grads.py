...
def process(args):
    ...
    buf = np.zeros((8, max_depth, n_net_samples_per_process), dtype=dtype) # V, V_tilde the first dimension
    ...
    for kk in range(n_net_samples_per_process):
        ...
        loss = nn.MSELoss(reduction="mean")(out, out_label)
        loss.backward()

        grads = np.empty([max_depth,n_data_samples, width], dtype=dtype) 
        q_mean = np.empty([max_depth], dtype=dtype) 
        q_var = np.empty([max_depth], dtype=dtype) 
        c_mean = np.empty([max_depth], dtype=dtype)
        c_var = np.empty([max_depth], dtype=dtype)

        for ii, pa in enumerate(model.pre_activations):
            grads[ii, :, :] = pa.grad.numpy() # pa.shape = (n_data_samples, width)
            var = np.var(pa.detach().numpy(),axis=1, keepdims=False) # shape = (n_data_samples)
            q_mean[ii] = np.mean(var) 
            q_var[ii] = np.var(var)
            corr_matrix = np.triu(np.corrcoef(pa.detach().numpy(), pa.detach().numpy(), rowvar=True), k=-1) # shape = (n_data_samples, n_data_samples)
            flattened_corr_matrix = corr_matrix[corr_matrix != 0]
            c_mean[ii] = np.mean(flattened_corr_matrix)
            c_var[ii] = np.var(flattened_corr_matrix)
            
        # average values and variances (w.r.t. dataset at last)
        grads = np.flip(grads, axis=0) # grads.shape = [max_depth, n_data_samples, width]
        grads_mean_dataset = np.mean(grads**2, axis=1, keepdims=False) # shape = [max_depth, width]
        grads_squaredmean_wandb = np.mean(grads**2, axis=2, keepdims=False) # shape = [max_depth, n_data_samples]
        buf[0,:,kk] = np.mean(grads_mean_dataset, axis=1, keepdims=False) # mean over wandb of mean dataset gradients
        buf[1,:,kk] = np.var(grads_mean_dataset, axis=1, keepdims=False) # variance over wandb of mean dataset gradients
        buf[2,:,kk] = np.mean(grads_squaredmean_wandb, axis=1,  keepdims=False) # mean over dataset of the mean squared wandb gradients
        buf[3,:,kk] = np.var(grads_squaredmean_wandb, axis=1,  keepdims=False) # variance over dataset of the mean squared wandb gradients
        buf[4,:,kk] = q_mean
        buf[5,:,kk] = q_var
        buf[6,:,kk] = c_mean
        buf[7,:,kk] = c_var
...
if my_colour == 0:
    ...
    # aggregate
    global_sum = np.zeros((8, max_depth), dtype=dtype)
    sub_comm.Reduce(local_sum, (global_sum, 8*max_depth, MPI.FLOAT), op=MPI.SUM, root=0)
    
    # aggregation of the result for each sub communicator
    if my_sub_rank==0: # colour 0 process 0 (i.e. global 0)
        # save 0th buffer
        save_buf = np.zeros((8, max_depth, n_b,n_w), dtype=dtype)
        ...
        for sub_colour in range(1,n_b * n_w):
            ...
            rcv_buf = np.zeros((8, max_depth), dtype=dtype)
            comm.Recv([rcv_buf, 8*max_depth, MPI.FLOAT], source=sub_colour*n_net_processes, tag=sub_colour) 
            save_buf[:, :,id_vb, id_vw] = rcv_buf

        # save data
        ut.save_grads(save_buf, f"data/grads_{act_func}_D{cfg['max_depth']}_W{cfg['width']}.h5", cfg)
    ...
    # aggregation of the result for each sub communicator
    if my_sub_rank==0:
        comm.Send([global_mean,8*max_depth, MPI.FLOAT], dest=0, tag=my_colour)
